...
private JLabel redLabel;
  ...
  public ColorSelect2() {
   ...
   this.setResizable(false); //窗体不能改变大小
    ...
    p.add(redLabel = new JLabel("红 "+SDef.SG_R));
    p.add(red = new JScrollBar(Adjustable.HORIZONTAL,  SDef.SG_R, 0, 0, 255));
    red.setBlockIncrement(16);
    red.addAdjustmentListener(this);

    p.add(greenLabel = new JLabel("绿 "+SDef.SG_G));
    p.add(green = new JScrollBar(Adjustable.HORIZONTAL,  SDef.SG_G, 0, 0, 255));
    green.setBlockIncrement(16);
    green.addAdjustmentListener(this);

    p.add(blueLabel = new JLabel("蓝 "+SDef.SG_B));
    p.add(blue = new JScrollBar(Adjustable.HORIZONTAL,  SDef.SG_B, 0, 0, 255));
    blue.setBlockIncrement(16);
    blue.addAdjustmentListener(this);

    contentPane.add(p, "South");
    colorPanel = new JPanel();
    colorPanel.setBackground(new Color(SDef.SG_R, SDef.SG_G, SDef.SG_B));
  ...
  public void adjustmentValueChanged(AdjustmentEvent evt) {
    ...
    colorPanel.setBackground(new Color(red.getValue(), green.getValue(),
                                       blue.getValue()));
    colorPanel.repaint();
    SDef.SG_R = red.getValue();
    SDef.SG_G = green.getValue();
    SDef.SG_B = blue.getValue();
  }
